...
export function mountGui(GUI, { params, state, info, actions }) {
  const { replay, refit, styleMeta, setThreshold, rebuildText } = actions;
  ...
  scroll.add(params, "scrollCooldown", 100, 800, 10).name("wheel cooldown");
  scroll.add(params, "damping", 0.5, 0.999, 0.001);
  scroll.add(params, "maxSpeed", 0.5, 60, 0.5);
  scroll.add(params, "dragSpeed", 0, 5, 0.01);
  scroll.add(params, "dragThreshold", 1, 32, 1).name("drag threshold");
